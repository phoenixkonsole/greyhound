...
if (buf = AllocMem(sizeof(struct cbbuf),MEMF_ANY|MEMF_CLEAR))
    {
    ...
    if (buf->mem = AllocMem(length+1L,MEMF_ANY|MEMF_CLEAR))
        {
        buf->size = length+1L;
    ...
    if (CBQueryFTXT(ior))
        ...
        while (buf=CBReadCHRS(ior))
              ...
              /* Process data */
	      	   ...
	      	   string = strdup((const char *)buf->mem);
...
if (ior = CBOpen(0L))
    ...
    if (!(CBWriteFTXT(ior,string)))
        {
        printf("Error writing to clipboard: io_Error = %d\n",ior->io_Error);
        }    
    CBClose(ior);
    }
else
