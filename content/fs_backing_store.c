	    ...
	    enum backing_store_flags flags)
{
	...
	case 1:
		greyhound_mkpath(&fname, NULL, 3, state->path, dat,
			       b32u_i);
		break;

	case 2:
		greyhound_mkpath(&fname, NULL, 4, state->path, dat,
			       b32u_d[0],
			       b32u_i);
		break;

	case 3:
		greyhound_mkpath(&fname, NULL, 5, state->path, dat,
			       b32u_d[0], b32u_d[1],
			       b32u_i);
		break;

	case 4:
		greyhound_mkpath(&fname, NULL, 6, state->path, dat,
			       b32u_d[0], b32u_d[1], b32u_d[2],
			       b32u_i);
		break;

	case 5:
		greyhound_mkpath(&fname, NULL, 7, state->path, dat,
			       b32u_d[0], b32u_d[1], b32u_d[2], b32u_d[3],
			       b32u_i);
		break;

	case 6:
		greyhound_mkpath(&fname, NULL, 8, state->path, dat,
			       b32u_d[0], b32u_d[1], b32u_d[2], b32u_d[3],
			       b32u_d[4],
			       b32u_i);
		break;
	...
	case 7:
		greyhound_mkpath(&fname, NULL, 9, state->path, dat,
			       b32u_d[0], b32u_d[1], b32u_d[2], b32u_d[3],
			       b32u_d[4], b32u_d[5],
			       b32u_i);
		break;
...
static nserror write_entries(struct store_state *state)
{
	...
	ret = greyhound_mkpath(&fname, NULL, 2, state->path, "entries");
	if (ret != NSERROR_OK) {
		return ret;
	}
...
write_cache_tag(struct store_state *state)
{
	...
	ret = greyhound_mkpath(&fname, NULL, 2, state->path, "CACHEDIR.TAG");
	if (ret != NSERROR_OK) {
		return ret;
	}
	...
	fprintf(fcachetag,
		...
		"# This file is a cache directory tag created by Greyhound.\n"
...
write_control(struct store_state *state)
{
	...
	ret = greyhound_mkpath(&fname, NULL, 2, state->path, "control");
	if (ret != NSERROR_OK) {
		return ret;
	}
