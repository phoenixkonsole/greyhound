		...
		nsfb_colour_t c, int *r_error, int *g_error, int *b_error)
{
	...
	int dr2, dg2, db2;  /* delta red, green blue values */
	...
	case NSFB_PALETTE_NSFB_8BPP:
		...
		dg = ((g * 7) + 128) >> 8;
		db = ((b * 4) + 128) >> 8;
		best_col = 40 * dr + 5 * dg + db;

		palent = palette->data[best_col];
		dr2 = r - ( palent        & 0xFF);
		dg2 = g - ((palent >> 8 ) & 0xFF);
		db2 = b - ((palent >> 16) & 0xFF);

		best_distance = square(dr2) + square(dg2) + square(db2);
		...
		dg = g - ((palent >>  8) & 0xFF);
		db = b - ((palent >> 16) & 0xFF);
		cur_distance = square(dr) + square(dg) + square(db);
...
static inline uint8_t nsfb_palette_best_match2(struct nsfb_palette_s *palette,
		nsfb_colour_t c, int *r_error, int *g_error, int *b_error)
{
	uint8_t best_col = 0;
