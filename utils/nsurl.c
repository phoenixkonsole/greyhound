...
/** \file
 * Greyhound URL handling (implementation).
 */
...
struct nsurl {
	...
	int count;	/* Number of references to Greyhound URL object */
...
/**
 * Create the components of a Greyhound URL object for a section of a URL string
 *
 * \param url_s		URL string
 ...
 * \param url		A Greyhound URL object, to which components may be added
	...
	case URL_HOST:
		...
		if (length != 0) {
			...
			if (!(flags & NSURL_F_NO_PORT)) {
				...
				/* Add non-redundant ports to Greyhound URL */
				sec_start = norm_start + colon - pegs->at +
						skip;
				if (!(flags & NSURL_F_NO_PORT) &&
 ...
 *
 * \param url		Greyhound URL components
 ...
 *
 * \param url		Greyhound URL object to set hash value for
 */
...
#ifdef NSURL_DEBUG
/**
 * Dump a Greyhound URL's internal components
 *
 * \param url	The Greyhound URL to dump components of
 */
...
nserror nsurl_create(const char * const url_s, nsurl **url)
{
	...
	/* Build Greyhound URL object from sections */
