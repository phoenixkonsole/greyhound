 ...
 *
 * SVG files are rendered to a Greyhound bitmap by creating a Cairo rendering
...
typedef struct rsvg_content {
	...
	struct bitmap *bitmap;	/**< Created Greyhound bitmap */
} rsvg_content;
...
/** Convert Cairo's ARGB output to Greyhound's favoured ABGR format.  It converts
 * the data in-place. 
 *
