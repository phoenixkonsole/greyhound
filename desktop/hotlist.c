 ...
 * Copyright 2013 Michael Drake <tlsa@greyhound-browser.org>
 *
...
static nserror hotlist_generate(void)
{
	...
	static const struct {
		...
		{ "http://www.greyhound-browser.org/",
				"HotlistHomepage" },
		{ "http://www.greyhound-browser.org/downloads/",
				"HotlistDownloads" },
		{ "http://www.greyhound-browser.org/documentation",
				"HotlistDocumentation" },
		{ "http://www.greyhound-browser.org/contact",
				"HotlistContact" }
	};
	const int n_entries = sizeof(default_entries) /
			sizeof(default_entries[0]);

	/* First make "Greyhound" folder for defualt entries */
	title = "Greyhound";
...
nserror hotlist_add_entry(nsurl *url, const char *title, bool at_y, int y)
{
	...
	if (url == NULL) {
		err = nsurl_create("http://greyhound-browser.org/", &url);
		if (err != NSERROR_OK) {
			return err;
		}
		assert(url != NULL);
