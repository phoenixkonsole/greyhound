		...
		lwc_string *mime_type)
{
	...
	ret = greyhound_mkpath(&fname, NULL, 2, ctx->path, leafname);
	if (ret != NSERROR_OK) {
		...
		hlcache_handle *c, bool index)
{
	...
	ret = greyhound_mkpath(&fname, NULL, 2, ctx->path, filename);
	if (ret != NSERROR_OK) {
...
static bool save_complete_inventory(save_complete_ctx *ctx)
{
	...
	ret = greyhound_mkpath(&fname, NULL, 2, ctx->path, "Inventory");
	if (ret != NSERROR_OK) {
		return false;
	}
